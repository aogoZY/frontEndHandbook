...
从[JWT官方Introduction](https://jwt.io/introduction/) 的官方介绍文档中看到这样一句话：
...
所以OAuth 在这里不过多讨论，相关信息同样可以参考上面那篇文章：[不要用JWT替代session管理（上）：全面了解Token,JWT,OAuth,SAML,SSO](https://juejin.im/post/5b3b870a5188251ac85826b8#heading-3)
...
（1）TGC 过期规则是什么，如何设置，设置多少比较合适？

跟用户登录状态是否失效相关的对象有两个，一个是session 内容的过期时间，就是app_1 使用ST 从CAS Server 换回的东西，我们不妨称之为token。token 失效之前，用户可以直接进行访问。一旦token 失效，用户浏览器会重定向至CAS Server 进行认证，CAS Server 又可以通过TGC 是否过期来判断用户是否登录，如果没过期则不需要登录。也就是说，**只要token 和TGC 有一个没过期，用户在单点登录的各个系统之间互相跳转访问是感知不到登录的**。但是我们日常工作中遇到的情况是：可能半个小时不操作就需要重新登录。此时，**我们感知到的情况似乎是，我们是否登录完全由token 来决定。**

未验证信息：TGC 的过期规则可以在cas.properties中进行配置，默认八小时，无操作，默认两小时。
