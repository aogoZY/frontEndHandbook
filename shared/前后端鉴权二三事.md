...
从[JWT官方Introduction](https://jwt.io/introduction/) 的介绍文档中看到这样一句话：
...
关于JWT 和SAML 的对比，有一张很有意思的图片，同样来自于[JWT官方Introduction](https://jwt.io/introduction/) 
...
举例来说，[华为云DevCloud](https://www.huaweicloud.com/devcloud/) 下有那么多微服务，比如项目管理、代码托管、代码检查、流水线、编译构建、部署、自动化测试等，用户如果在使用任意一个服务没有登录的时候都可以去同一个地方进行登录认证，登录之后的一段时间内可以无需登录访问所有其他服务。
...
- TGC 跟TGT 相关联，是用户浏览器直接向CAS Server 获取ST 的票据，如果TGC 有效，用户就不需要完成表单信息填写的步骤直接实现登录
- TGC 的过期策略是这样设置的，如果用户一直没有页面操作和后台接口请求，那么默认2 小时过期，如果一直有操作，默认8 小时过期，开发者可以在[cas.properties](https://apereo.github.io/cas/5.0.x/installation/Configuration-Properties.html#cas-properties) 中对这两个过期时间进行修改，一般的应用中不会配置这么长的过期时间
```
...
（2）业务服务器通过在session 中放置token 的方式，保证用户登录的一段时间内不需要再次登录。那么如何保证各个业务的sessionId 不冲突？


（3）有没有可能出现这种情况，我在猛犸的登录没过期，在devcloud的登录过期了？

CAS 协议：https://apereo.github.io/cas/4.2.x/protocol/CAS-Protocol.html

过期时间Ref：https://apereo.github.io/cas/4.2.x/installation/Configuring-Ticket-Expiration-Policy.html#multitimeuseortimeoutexpirationpolicy

Ref：https://blog.csdn.net/yushenzaishi/article/details/80435343
